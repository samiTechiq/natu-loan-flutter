...
class LoanController extends GetxController {
  ...
  Future<void> createLoan() async {
    ...
    try {
      ...
      final response = await _cacheService.createLoan(
      ...
      final message =
          response['message'] ?? 'Loan created successfully Hurray!';
      Get.snackbar(
        'Success',
        message,
        snackPosition: SnackPosition.BOTTOM,
  ...
  Future<void> refreshStaffList() async {
    ApiClient().invalidateCache("/transactions/users", 'GET');
    try {
      isLoading.value = true;
      await loadStaffList();
    } catch (e) {
      print('Error refreshing staff list: $e');
      Get.snackbar(
        'Error',
        'Failed to refresh staff list: $e',
        snackPosition: SnackPosition.BOTTOM,
        backgroundColor: Colors.red.shade100,
        colorText: Colors.red.shade800,
      );
